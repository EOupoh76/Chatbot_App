...
# Chargement du texte de Sherlock Holmes
...
def chatbot(query, text):
    ...
    relevant_sentence = get_most_relevant_sentence(query, cleaned_sentences)
    response = f"Based on your question, here's what Sherlock Holmes would say: '{relevant_sentence}'"
    return response


# Fonction principale Streamlit
def main():
    st.title("Sherlock Holmes Chatbot")
    query = st.text_input("Ask a question to Sherlock Holmes:")
    if st.button("Ask"):
        response = chatbot(query, sherlock_holmes_text)
        st.text(response)
